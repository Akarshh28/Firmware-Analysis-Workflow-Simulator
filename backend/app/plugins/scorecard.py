...
class ScorecardPlugin(BaseToolPlugin):
    @property
    def name(self) -> str:
        return "scorecard"
    ...
    def documentation(self) -> Dict[str, Any]:
        return {
            "purpose": "Calculate overall risk score (CVSS) based on findings.",
            ...
            "internal_working": "Executes 'scorecard' on the target.",
            "commands": [
                {
                    "command": "scorecard --help",
                    "explanation": "Display help for scorecard"
                }
            ],
    ...
    async def execute_simulated(self, tool_input: ToolInput) -> ToolOutput:
        ...
        return ToolOutput(
            success=True,
            exit_code=0,
            logs=[{"log_type": "SYSTEM", "message": "Simulated execution of scorecard completed."}],
            generated_files=[],
            parsed_metrics={}
        )
